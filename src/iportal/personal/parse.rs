use crate::{error::parse_err, iportal::{personal::{PersonalDataItem, PersonalDataTypeEnum}, util::iportal_jsondata_precheck}};
...
pub fn parse_personal_data_query_ids(
    ...
    let arr = json_value?.get("data").ok_or(parse_err("JSON解析错误", json_str))?.as_array().ok_or(parse_err("JSON解析错误", json_str))?.clone();
...
pub fn parse_personal_data(
    ...
    let json_value: Result<serde_json::Value, crate::Error<crate::cas::error::TokenExpired>> = iportal_jsondata_precheck(json_str);
    let data_item = json_value?.get("data").ok_or(parse_err("JSON解析错误", json_str))?.clone();
